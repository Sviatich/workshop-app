document.addEventListener('DOMContentLoaded', () => {
  ...
  function attachPartySuggest(input, onPick) {
    if (!input) return;
    ...
    function render(items) {
      ...
      for (const it of items) {
        ...
        b.addEventListener('click', () => {
          try { onPick({ name, inn, kpp, addr, data: it.data }); } catch (_) {}
          hide();
          input.blur();
        });
        dd.appendChild(b);
    ...
    // Hide on payer type change (radios)
    Array.from(document.querySelectorAll('input[name="payer_type"]')).forEach(r => r.addEventListener('change', hide));

    input.addEventListener('input', onInput);
    input.addEventListener('focus', onInput);
  }

  // INN suggestions -> fill INN and company name
  attachPartySuggest(innInput, ({ name, inn }) => {
    if (innInput) innInput.value = inn || '';
    const comp = document.getElementById('company_name');
    if (comp && name) comp.value = name;
    if (innInput) innInput.dispatchEvent(new Event('input'));
  });

  // Company name suggestions -> fill company name and INN
  attachPartySuggest(companyInput, ({ name, inn }) => {
    if (companyInput) companyInput.value = name || '';
    const innEl = document.getElementById('inn');
    if (innEl && inn) innEl.value = inn;
    if (companyInput) companyInput.dispatchEvent(new Event('input'));
  });
});
